...
#10.篩選備註欄異常值-----
#1.	急買、急賣
急買急賣<- nrow(df[grepl("急買|急賣|急售|急購|急|", df$備註),])
#2.	債權、債務
債權債務<- nrow(df[grepl("債權|債務|借貸|債", df$備註),])
#3.	親友、兄、哥、弟、姊、姐、妹、父、母、孫、女、夫、妻、配偶、關係人、
#股東、員工、共有人、親等、占用戶、親屬
關係人<-nrow(df[grepl("親友|摯友|友|朋友|兄|哥|弟|姊|姐|妹|父|母|孫|女|夫|妻|舅|甥|姨|姪|叔|嬸|姑|伯|
                   配偶|姻親|婚姻|近親|一等|1等|二等|三等|四等|直系|旁系|繼承|表親|血親|離婚|婚|
                   關係人|股東|員工|共有人|親等|占用戶|親屬|等親|關係|親|承租人|鄰居|承租戶|房東|房客|租客|地主戶|新屬", df$備註),])
#4.	合併使用、併同出售
合併使用<-nrow(df[grepl("合併使用|併同出售|一併|合併|併", df$備註),])
#5.	糾紛、爭議
糾紛爭議<-nrow(df[grepl("糾紛|爭議|訴訟|占用|調解|佔用", df$備註),])
#6.	標售、讓售
標售讓售<- nrow(df[grepl("標售|讓售|標購|讓購", df$備註),])
#7.	預售
預售<-nrow(df[grepl("預售|預售屋|預售價格|預售案件", df$備註),])
#8.	瑕疵、傾斜、死亡、凶宅、海砂、輻射、地下室
特殊建築<-nrow(df[grepl("瑕疵|瑕疵物件|瑕疵擔保|傾斜|死亡|凶宅|海砂|輻射|地下室|
                    自殺|他殺|兇宅|身故|亡故|壁癌|滲水|漏水|滲漏|璧癌|火災|年久失修|毀損|破敗", df$備註),])
#9.	持分
持分<-nrow(df[grepl("持分|持份|持分移轉|持分交易|共同持分|持分買賣|持分面積", df$備註),])
#10.	頂樓加蓋
頂樓加蓋<-nrow(df[grepl("頂樓加蓋|頂加|加蓋|頂樓鐵皮|加建|頂樓加建|頂增", df$備註),])
#11.	土地建物分別登記
土地建物分別登記<-nrow(df[grepl("土地建物分別登記|分次登記|分別登記", df$備註),])
#12.  畸零地
畸零地<-nrow(df[grepl("畸零地|畸零|綺零|零地", df$備註),])
#13.  公共設施保留地
公共設施保留地<-nrow(df[grepl("公共設施保留地|公共設施|保留地", df$備註),])
#14.  建商合建
建商合建<-nrow(df[grepl("建商合建|合建", df$備註),])
#15.  受民情風俗因素影響
受民情風俗因素影響<-nrow(df[grepl("民情風俗因素影響|民情|風俗", df$備註),])
#16.  團購 
團購 <-nrow(df[grepl("團購|團購價|團購交易|團購買賣優惠戶", df$備註),])
#17.	農作物、農業設施、農舍
農業設施<-nrow(df[grepl("農作物|農業設施|農舍|農|農業", df$備註),])
#18.	陽台外推、增建、夾層
增建<-nrow(df[grepl("陽台外推|增建|夾層", df$備註),])
#19. 其他異常值
其他異常值<-nrow(df[grepl("托兒所|墳墓|靈骨塔|塔位|法院|法拍|大陸|中國|
                特殊|廠房|非自然|意外|貳欽等|贈與|返還|返環|特惠|道路|用地|保育|畜牧|原住民|
                連件辦理|連件交易|連件|公司清算|倉庫|辦公室|事務所|現金|公設保留|國有財產|
                附條件買賣|附條件之買賣|多戶議價團購|不具實價參考意義|租約|租賃契約|屋況極差|出清|特殊|
                屋況不佳|屋況瑕疵|查封|退輔會|承購|低於行情|屋況不好|屋況較差|屋況老舊|民宿|土地持分|小木屋|
                老舊|破舊|斷水斷電|沒門沒窗|建方與地主之買賣|持分|比較便宜", df$備註),])
...
備註異常值分類統計 <- data.frame(rbind(急買急賣,債權債務,關係人,合併使用,糾紛爭議,標售讓售,預售,特殊建築,
                                持分,頂樓加蓋,土地建物分別登記,畸零地,公共設施保留地,建商合建,
                                受民情風俗因素影響,團購,農業設施,增建,其他異常值,備註刪除筆數))
df <- df[!grepl("急買|急賣|急售|急購|急|
                債權|債務|借貸|債|
                親友|摯友|友|朋友|兄|哥|弟|姊|姐|妹|父|母|孫|女|夫|妻|舅|甥|姨|姪|叔|嬸|姑|伯|
                配偶|姻親|婚姻|近親|一等|1等|二等|三等|四等|直系|旁系|繼承|表親|血親|離婚|婚|
                關係人|股東|員工|共有人|親等|占用戶|親屬|等親|關係|親|承租人|鄰居|承租戶|房東|房客|租客|地主戶|新屬|
                合併使用|併同出售|一併|合併|併|
                糾紛|爭議|訴訟|占用|調解|佔用|
                標售|讓售|標購|讓購|
                預售|預售屋|預售價格|預售案件|
                瑕疵|瑕疵物件|瑕疵擔保|傾斜|死亡|凶宅|海砂|輻射|地下室|
                自殺|他殺|兇宅|身故|亡故|壁癌|滲水|漏水|滲漏|璧癌|火災|年久失修|毀損|破敗|
                持分|持份|持分移轉|持分交易|共同持分|持分買賣|持分面積|
                頂樓加蓋|頂加|加蓋|頂樓鐵皮|加建|頂樓加建|頂增|
                土地建物分別登記|分次登記|分別登記|
                畸零地|畸零|綺零|零地|
                公共設施保留地|公共設施|保留地|
                建商合建|合建|
